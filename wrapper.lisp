...
(defclass metadata ()
  ((version :initform NIL :reader version)
   ...
   (track :initform NIL :reader track)
...
(defmethod initialize-instance :after ((data metadata) &key id3v2 id3v1)
  ...
  (with-slots (version title artist album track year genre comments texts extras pictures) data
    (when id3v1
      (setf version "1.0")
      ...
      (setf year (parse-integer (direct-str (cl-mpg123-cffi:id3v1-year id3v1) 4)))
      (let ((cptr (cl-mpg123-cffi:id3v1-comment id3v1))
        ...
        (cond ((= 0 (mem-ref cptr :char 28))
               ...
               (setf track (mem-ref cptr :char 29)))
              (T
               (setf comment (direct-str cptr 30))))
        (when comment (push (list "???" "id3v1 comment" comment) comments)))
      (push (id3v1-genre (cl-mpg123-cffi:id3v1-genre id3v1)) genre))
    (when id3v2
      ...
      (do-text-array (lang id desc text)
        ...
        (cond ((string= id "TIT2") (setf title text))
              ((string= id "TPE1") (setf artist text))
              ((string= id "TALB") (setf album text))
              ((string= id "TRCK") (setf track (parse-integer text)))
              ((string= id "TDRC") (setf year (parse-integer text)))
              ((string= id "TCON") (dolist (g (id3v2-genre text))
                                     (pushnew g genre :test #'string-equal)))
              (T (push (list id desc text) texts))))
      (do-text-array (lang id desc text)
          ((cl-mpg123-cffi:id3v2-comment-list id3v2)
           (cl-mpg123-cffi:id3v2-comments id3v2))
        (declare (ignore id))
        (push (list lang desc text) texts))
      (do-text-array (lang id desc text)
